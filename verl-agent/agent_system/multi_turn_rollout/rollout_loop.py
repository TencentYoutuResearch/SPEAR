...
class TrajectoryCollector:
    def __init__(self, config, tokenizer: PreTrainedTokenizer, processor=None):
        ...
        self.snapshots_root_dir = "sokoban_snapshots"
    ...
    def preprocess_single_sample(
        ...
        """
        # here comes the analysis of gen_batch
        """
        gen_batch.batch:
            ['input_ids', 'attention_mask', 'position_ids']
        gen_batch.non_tensor_batch:
            ['raw_prompt_ids', 'data_source', 'multi_modal_data', 'raw_prompt', 'tools_kwargs']
        """
        ...
        if is_multi_modal:
            ...
            # image_tensor = image_inputs['pixel_values']  ## torch.Size([324, 1176]); already processed by the tokenizer of VLM
            image_grid_thw = deepcopy(image_inputs["image_grid_thw"])
    ...
    def vanilla_multi_turn_loop(
        ...
        if "val_data_dir" in gen_batch.meta_info:
            val_data_dir = gen_batch.meta_info["val_data_dir"]
            val_data_root = os.path.join(val_data_dir, f"images-step_{global_steps}")
            os.makedirs(val_data_root, exist_ok=True)
        else:
            val_data_root = None
        ...
        for _step in range(self.config.env.max_steps):
            ...
            ### ['input_ids', 'attention_mask', 'position_ids'])
            ### ['multi_modal_data', 'multi_modal_inputs', 'raw_prompt_ids', 'anchor_obs', 'index', 'data_source', 'raw_prompt']
            batch_keys_to_pop = ["input_ids", "attention_mask", "position_ids"]
            non_tensor_batch_keys_to_pop = ["raw_prompt_ids"]
            if "multi_modal_data" in batch.non_tensor_batch:
                multi_modal_data = deepcopy(batch.non_tensor_batch["multi_modal_data"])
                non_tensor_batch_keys_to_pop.append("multi_modal_data")
                if val_data_root:
                    for multi_modal_data_item, uid_item, traj_item in zip(
                        multi_modal_data, uid_batch, traj_uid, strict=False
                    ):
                        val_data_root_uid_traj_id = os.path.join(val_data_root, f"{uid_item}", f"{traj_item}")
                        os.makedirs(val_data_root_uid_traj_id, exist_ok=True)
                        images_item = multi_modal_data_item["image"]
                        for image_item_id, image_item in enumerate(images_item):
                            image_item.save(
                                os.path.join(val_data_root_uid_traj_id, f"step-{_step}-image-{image_item_id}.png")
                            )
    ...
    def dynamic_multi_turn_loop(
        ...
        total_success = {
            key: np.concatenate([success[key] for success in total_success], axis=0) for key in total_success[0].keys()
        }
    ...
    def multi_turn_loop(
        ...
        raw_prompt_list = gen_batch_output.non_tensor_batch["raw_prompt"]
        output_ids = gen_batch_output.batch["responses"]
        output_ids_wo_padding = [
            [
                token_id
                for token_id in ids
                if (token_id != self.tokenizer.pad_token_id) and (token_id != self.tokenizer.eos_token_id)
            ]
            for ids in output_ids
        ]
        output_texts = [self.tokenizer.decode(ids, skip_special_tokens=False) for ids in output_ids_wo_padding]
        messages = []
        assert len(raw_prompt_list) == len(output_texts)
        for raw_prompt, output_text in zip(raw_prompt_list, output_texts, strict=False):
            raw_prompt_cp = deepcopy(raw_prompt)
            if type(raw_prompt_cp) is not list:
                raw_prompt_cp = raw_prompt_cp.tolist()
            assert raw_prompt_cp[-1]["role"] == "user"
            raw_prompt_cp.append({"role": "assistant", "content": output_text.strip()})
            messages.append(raw_prompt_cp)
        assert len(messages) == len(raw_prompt_list)
        ### must convert to numpy array for the following masking operation
        gen_batch_output.non_tensor_batch["messages"] = np.array(messages)
        return gen_batch_output
