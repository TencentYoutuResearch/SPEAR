...
def make_envs(config):
    ...
    if "gym_cards" in config.env.env_name.lower():
        ...
        assert(os.path.exists(attr_path)), f"Attr path {attr_path} does not exist"
        ...
        print(">>> webshop configures", env_kwargs)
        print(">>> resources_per_worker", resources_per_worker)
        ...
        import time
        time_sleep = (config.data.train_batch_size * group_n + config.data.val_batch_size) * 0.1
        print(f"Now wait {time_sleep}s for the envs to be ready")
        time.sleep(time_sleep) # wait for the envs to be ready
        return envs, val_envs
