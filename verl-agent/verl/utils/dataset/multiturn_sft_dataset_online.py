...
class MultiTurnSFTDatasetOnline(Dataset):
    """
    Dataset for multi-turn conversations where each assistant response should be trained
    """

    def __init__(self, messages, tools, tokenizer, config=None):
        # Set defaults and extract parameters from config if provided
        config = config or {}
        # self.truncation = config.data.get("truncation", "error")
        self.truncation = "right"
        self.max_length = config.data.max_length
        print("Online SFT dataset max length", self.max_length)
    ...
    def _validate_and_convert_tokens(
        ...
        if len(concat_tokens) != len(full_tokens_list) or not all(a == b for a, b in zip(concat_tokens, full_tokens_list, strict=False)):
            logging.warning(
                ...
                # f"concat tokens id: {concat_tokens_str}"
