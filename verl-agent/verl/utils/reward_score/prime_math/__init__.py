...
def split_tuple(expr: str):
    ...
    if len(expr) == 0:
        return []
    if len(expr) > 2 and expr[0] in TUPLE_CHARS and \
        expr[-1] in TUPLE_CHARS and \
...
def grade_answer(given_answer: str, ground_truth: str) -> bool:
    ...
    if len(ground_truth_elems) > 1 and \
        (ground_truth_normalized[0] != given_normalized[0] or \
            ground_truth_normalized[-1] != given_normalized[-1]) or \
                len(ground_truth_elems) != len(given_elems):
        is_correct = False
    else:
