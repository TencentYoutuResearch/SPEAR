...
def merge_megatron_ckpt_gptmodel(wrapped_models, config, dtype, is_value_model=False, tie_word_embeddings=False):
    ...
    for i, wrapped_model in enumerate(wrapped_models):
        ...
        assert len(models[i].decoder.layers) == num_layers_per_model, f"len model layers {len(models[i].decoder.layers)} not equal to num_layers_per_model {num_layers_per_model}"
