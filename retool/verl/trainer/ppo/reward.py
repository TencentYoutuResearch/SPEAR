...
def load_reward_manager(config, tokenizer, num_examine, **reward_kwargs):
    ...
    if compute_score is None:
        print("Compute Score is None, use Sandbox Manager (specifically for remote computation of rewards) 🎃")
        sandbox_config = config.reward_model.get("sandbox_fusion")
        sandbox_url = sandbox_config.get("url") if sandbox_config else None
        memory_limit_mb = sandbox_config.get("memory_limit_mb", 1024)
        if sandbox_url:
            print("Compute code rewards via sandbox manager 🏝️")
            ...
            print("Compute code rewards via default manager 🥹")
...
def compute_reward(data: DataProto, reward_fn, max_response_len=16384):
    """
    ...
    """
    try:
        reward_result = reward_fn(data, return_dict=True, max_response_len=max_response_len)
...
def compute_reward_async(data: DataProto, config=None, tokenizer=None, reward_fn=None, max_response_len=16384):
    """
