...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            self.save_messages_to_json(test_output_gen_batch, is_train=False)
    ...
    def save_messages_to_json(self, gen_batch_output, is_train=True):
        ...
        try:
            ...
            if is_train:
                print(">>>>>>>>>>>>>>>>>>>>>>>>>>gen_batch_output: fit<<<<<<<<<<<<<<<<<<<<<<<<<<")
                filename = f"messages_{time_stamp}_step_{self.global_steps}.jsonl"
            else:
                print(">>>>>>>>>>>>>>>>>>>>>>>>>>gen_batch_output: val<<<<<<<<<<<<<<<<<<<<<<<<<<")
                filename = f"val_messages_{time_stamp}_step_{self.global_steps}.jsonl"
