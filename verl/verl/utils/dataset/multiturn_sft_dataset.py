...
class MultiTurnSFTDataset(Dataset):
    ...
    def _download(self):
        for i, parquet_file in enumerate(self.parquet_files):
            print("Downloading parquet file:", parquet_file)
            self.parquet_files[i] = copy_local_path_from_hdfs(parquet_file, verbose=True)

    def convert_to_standard(self, messages):
        messages_new = []
    ...
    def _read_files_and_process(self):
        ...
        print("total dataframe length", len(self.dataframe))
        # Extract tools list from dataframe
        if self.tools_key in self.dataframe.columns:
            tools_candidate = self.dataframe[self.tools_key].apply(convert_nested_value_to_list_recursive).tolist()
            self.tools = []
        else:
            self.tools = None
        ...
        self.messages = []
        messages_candidate = self.dataframe[self.messages_key].apply(series_to_item).tolist()
        for item, messages in enumerate(tqdm(messages_candidate)):
            if self.tools is not None:
                tools = tools_candidate[item]
            else:
                tools = None
            messages_reform = self.convert_to_standard(messages)
            try:
    ...
    def _process_message_tokens(
        ...
        if is_assistant:
            ...
            if loss_mask_flag:
                ...
                loss_mask = [0] * (len(generation_prompt_tokens)) + [0] * (
                    len(message_tokens) - len(generation_prompt_tokens)
                )
        else:
    ...
    def __getitem__(self, item):
        ...
        messages = deepcopy(self.messages[item])
        ...
        while i < len(messages):
            ...
            if "loss" in cur_messages:
                loss_mask_flag = cur_messages.pop("loss")
            else:
                loss_mask_flag = True
            if "tool_calls" in cur_messages:
                if cur_messages["tool_calls"] is None:
        ...
        if item < 10:
            print(">>> detokenized text\n\n", self.tokenizer.decode(input_ids.tolist()), "-------------\n\n")
